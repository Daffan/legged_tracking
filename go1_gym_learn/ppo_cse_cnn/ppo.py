...
from go1_gym_learn.ppo_cse_cnn import RolloutStorage
from go1_gym_learn.ppo_cse_cnn import caches
...
class PPO:
    ...
    def process_env_step(self, rewards, dones, infos):
        ...
        # TODO: replace with dummy env bins
        self.transition.env_bins = torch.zeros_like(dones)  # infos["env_bins"]
    ...
    def update(self):
        ...
        for obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            for epoch in range(PPO_Args.num_adaptation_module_substeps):
                recurrent_latent = self.actor_critic.process_obs_history(obs_history_batch)
                adaptation_pred = self.actor_critic.adaptation_module(recurrent_latent)
                with torch.no_grad():
